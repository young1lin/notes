...
第一个大家都知道的，限定词用以限定名词所指的范围，对名词起泛指或特指、定量或不定量等限定修饰作用；而形容词则是用来表示名词的性质、特征的。

限定词一般不用作表语（少数表示数量的限定词few, many和much除外），而绝大多数形容词都可用作表语。除了few, little, many和much外，限定词没有比较级和最高级形式，而大多数形容词有比较级和最高级形式。
