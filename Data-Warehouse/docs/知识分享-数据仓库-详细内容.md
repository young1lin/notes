...
在 Flink 中，有界数据流（**Unbounded streams** ）处理就是流处理，无界数据流（**Bounded streams**）处理就是流处理。
下面更为直观的图。
![](https://static001.geekbang.org/resource/image/2f/b2/2f8e72ce532cf1d05306cb8b78510bb2.png)
这是官网的图
...
![流计算开发套件.png](https://i.loli.net/2021/10/08/9JGjlvItWbZcRkS.png)
...
![数仓相关内容-MySQL-Architecture.png](https://i.loli.net/2021/09/15/izG4Y9STDAnbPMC.png)
...
![数仓相关内容-传统数据模型适用于维表.png](https://i.loli.net/2021/09/14/8Oc57EhUubvAz9f.png)
...
-   **ETL** 解决方案
-   **统计分析、报告和数据挖掘**功能
-   用于向业务用户可视化和呈现数据的客户端分析工具
-   其他更复杂的分析应用程序，通过应用数据科学和人工智能 (AI) 算法或图形和空间特征生成可操作的信息，从而实现更多类型的大规模数据分析
...
![数仓相关内容-数据仓库和数据集市的关系.png](https://i.loli.net/2021/10/08/eLciGuEvDdP75OK.png)
...
![数仓相关内容-星形连接.png](https://i.loli.net/2021/09/14/5MsVjxidh19CFOy.png)
...
![数仓相关内容-星型-雪花结构.png](https://i.loli.net/2021/09/14/UAg6FDMbXrK5RaN.png)
...
// TODO 将自己编写的 Spring Batch Demo Project 贴上来
...
![数仓相关内容-网络流控.png](https://i.loli.net/2021/09/14/76reQOFYtSXs4Gg.png)
...
![数仓相关内容-静态限速.png](https://i.loli.net/2021/09/14/UoqATcLpFIaf2bK.png)
...
![数仓相关内容-动态反馈.png](https://i.loli.net/2021/09/14/OeLqiar3U74Zhs1.png)
...
![Pasted image 20210910112235.png](https://i.loli.net/2021/09/14/4nYPSCm82AHlFL9.png)
...
![Pasted image 20210910135021.png](https://i.loli.net/2021/09/14/rRv8up3kQO1jw5s.png)
...
和下面要讲的 Flink 的 DataStream 输出成 DataStreamSink 是类似的。
	![Pasted image 20210910135641.png](https://i.loli.net/2021/09/14/ti2nA7qUIWTRaeX.png)
...
![数仓相关内容-SparkStreaming反压.png](https://i.loli.net/2021/09/14/uh4BCQVFnD3J2tN.png)
...
配置信息如下
![flink-1.png](https://i.loli.net/2021/09/14/Axvp1OsQhC48EgT.png)
![flink-slot.png](https://i.loli.net/2021/09/14/Anp7r9OiJCdWm6R.png)
...
这张图就体现了 Flink 在做网络传输的时候基本的数据的流向，发送端在发送网络数据前要经历自己内部的一个流程，会有一个自己的 Network Buffer，在底层用 Netty 去做通信，Netty 这一层又有属于自己 ChannelOutbound Buffer，因为最终是要通过 Socket 做网络请求的发送，所以在 Socket 也有自己的 Send Buffer，同样在接收端也有对应的三级 Buffer。学计算机网络的时候我们了解到，TCP 是自带流量控制的。实际上 Flink （beforeV1.5）就是通过 TCP 的流控机制来实现 feedback 的。
